class ComprasController < ApplicationController
  ...
  # GET /compras/1.json
    def show
        respond_to do |format|
            format.html
            format.pdf do
               report = CompraPDF.create @compra
                
                
                send_data   report.generate, 
                            filename: "Orden#{@compra.pin}.pdf",
  ...
  private
    ...
    def compra_params
      params.require(:compra).permit(:pin, :empleado_id, :proyecto_id, :subtotal, :iva, :total, :aprobar, :proveedor_id, productos_attributes: [:id, :precio_compra, :unidad_compra, :descripcion_compra, :cantidad_compra, :total, :_destroy])
    end
    
    def render_compra_list(compra)
